 ...
 * </ul>
 */
public class NMediaPickerActivity extends NActivity implements
        NMediaSelectedListener, NCropListener, OnBackStackChangedListener, NFragmentHost {
...
//                            R.drawable.picker_actionbar_translucent));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (item == mPhoto) {
            ...
            if (!(activePage instanceof NMediaPickerFragment)) return true;
    ...
    private void syncMediaOptions() {
        ...
        if (mPhoto != null) {
            if (mMediaOptions.isCanTakePhoto())
    ...
    private RecursiveFileObserver.OnFileCreatedListener mOnFileCreatedListener = new RecursiveFileObserver.OnFileCreatedListener() {
        ...
        public void onFileCreate(File file) {
            if (mFilesCreatedWhileCapturePhoto == null)
                mFilesCreatedWhileCapturePhoto = new ArrayList<>();
            ...
            switch (requestCode) {
                case REQUEST_PHOTO_CAPTURE:
                    ...
                    if (mPhotoFileCapture != null) {
                        PhotoUtils.galleryAddPic(getApplicationContext(),
                                mPhotoFileCapture);
                        if (mMediaOptions.isCropped()) {
                            ...
                            NMediaItem item = new NMediaItem(NMediaItem.PHOTO,
                                    Uri.fromFile(mPhotoFileCapture));
                            ArrayList<NMediaItem> list = new ArrayList<>();
                            list.add(item);
                            returnBackData(list);
                        }
                    }
    ...
    public void onSuccess(NMediaItem mediaItem) {
        List<NMediaItem> list = new ArrayList<>();
        list.add(mediaItem);
        returnBackData(list);
    }
    ...
    private int checkValidVideo(Uri videoUri) {
        ...
        long duration = VideoUtils.getDuration(getApplicationContext(),
                VideoUtils.getRealPathFromURI(getContentResolver(), videoUri));
        if (duration == 0) {
    ...
    public NavigationManager getNavigationManager() {
        //TODO create the Navigation Manager ( if needed )
        return null;
    }
}
