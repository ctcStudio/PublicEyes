...
public class NMediaOptions implements Parcelable {
    ...
    private NMediaOptions(Builder builder) {
        ...
        this.canTakePhoto = builder.canTakePhoto;
    }
    ...
    public static class Builder {
        ...
        /**
         * @param croppedFile Use for save image that cropped. Default image cropped
         *                    saved in file that created by
         *                    {@link NMediaUtils#createDefaultImageFile()}
         *                    <p/>
        ...
        public Builder canTakePhoto(boolean canTakePhoto) {
    ...
    public void writeToParcel(Parcel dest, int flags) {
        dest.writeByte(this.canSelectMultiPhoto ? (byte) 1 : (byte) 0);
        dest.writeByte(this.canSelectMultiVideo ? (byte) 1 : (byte) 0);
        dest.writeByte(this.isCropped ? (byte) 1 : (byte) 0);
        dest.writeInt(this.maxVideoDuration);
        dest.writeInt(this.minVideoDuration);
        dest.writeByte(this.canSelectPhoto ? (byte) 1 : (byte) 0);
        dest.writeByte(this.canSelectVideo ? (byte) 1 : (byte) 0);
        dest.writeSerializable(this.photoCaptureFile);
        dest.writeInt(this.aspectX);
        dest.writeInt(this.aspectY);
        dest.writeByte(this.fixAspectRatio ? (byte) 1 : (byte) 0);
        dest.writeSerializable(this.croppedFile);
        dest.writeTypedList(this.mediaListSelected);
        dest.writeByte(this.showWarningVideoDuration ? (byte) 1 : (byte) 0);
        dest.writeByte(this.canTakePhoto ? (byte) 1 : (byte) 0);
    }

    protected NMediaOptions(Parcel in) {
        this.canSelectMultiPhoto = in.readByte() != 0;
        this.canSelectMultiVideo = in.readByte() != 0;
        this.isCropped = in.readByte() != 0;
        this.maxVideoDuration = in.readInt();
        this.minVideoDuration = in.readInt();
        this.canSelectPhoto = in.readByte() != 0;
        this.canSelectVideo = in.readByte() != 0;
        ...
        this.mediaListSelected = in.createTypedArrayList(NMediaItem.CREATOR);
        this.showWarningVideoDuration = in.readByte() != 0;
        this.canTakePhoto = in.readByte() != 0;
    }
