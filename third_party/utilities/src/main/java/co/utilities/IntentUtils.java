...
/**
 * All utils for Intent. Intent for send email, sms... <br/>
 * References: https://github.com/d-tarasov/android-intents
 */
public class IntentUtils {
    ...
    /**
     * Refers from: http://stackoverflow.com/a/21557967
     *
     * @param latitude
     * @param longitude
     * @param zoomLevel
     ...
     * @return
     */
    public static Intent showLocationWithMarker(float latitude, float longitude,
                                                Integer zoomLevel, String label) {
        Intent intent = new Intent();
        intent.setAction(Intent.ACTION_VIEW);
        String data = String.format("geo:%s,%s?q=%s,%s(%s)", latitude, longitude, latitude, longitude, label);
        if (zoomLevel != null) {
            data = String.format("%s?z=%s", data, zoomLevel);
        }
        intent.setData(Uri.parse(data));
     ...
     * @return
     */
    public static Intent showLocationWithBrowser(float latitude, float longitude) {
        Intent intent = new Intent();
        intent.setAction(Intent.ACTION_VIEW);
        String data = String.format("http://maps.google.com/maps?q=%s,%s&iwloc=A&hl=es", latitude, longitude);
        intent.setData(Uri.parse(data));
        return intent;
    }
